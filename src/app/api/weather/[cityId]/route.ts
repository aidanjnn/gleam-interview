...
import { getCityById, CITIES } from '../../config/cities';

// GET /api/weather/[cityId] - Returns weather data for a specific city
export async function GET(
    ...
    if (!city) {
        return NextResponse.json(
            { error: `City not found: ${cityId}`, availableCities: CITIES.map(c => c.id) },
            { status: 404 }
        );
    }
    ...
    try {
        ...
        const transformedData = transformWeatherData(city, data);
